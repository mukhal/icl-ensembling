...
class MetaICLModel(object):
    ...
    def load(self, checkpoint=None, gpt2="gpt2-large"):
        ...
        if checkpoint is None:
            if gpt2.startswith("gpt2"):
                model = AutoModelForCausalLM.from_pretrained(gpt2, cache_dir=os.environ["TRANSFORMERS_CACHE"], torch_dtype=torch.float16 if self.fp16 else torch.float32)
            elif "gpt-j" in gpt2:
                model = AutoModelForCausalLM.from_pretrained("EleutherAI/gpt-j-6B", cache_dir=os.environ["TRANSFORMERS_CACHE"], torch_dtype=torch.float16 if self.fp16 else torch.float32)
            elif "llama" in gpt2:
            ...
            model = AutoModelForCausalLM.from_pretrained(gpt2, state_dict=state_dict, 
                    cache_dir= os.environ["TRANSFORMERS_CACHE"], torch_dtype=torch.float16 if self.fp16 else torch.float32)
        
        ## print model config 
        self.logger.info("Model config: %s" % model.config)
    ...
    def do_predict(self, data, batch_size=1, losses=None, verbose=False, get_probs=False):
        if losses is None:
            losses = self.do_inference(data, batch_size, verbose=verbose)
        losses = np.array(losses)
        assert len(losses)==len(data)

        losses = losses.reshape((-1, data.n_ensemble)) # (n_examples, n_ensemble)
        predictions = []
        per_label_probs = []
        per_ensemble_ll = []

        for _, dp in enumerate(data.metadata):
            per_ensemble_log_likelihood = [losses[idx[0]] * -1 for idx in dp["indices"]]
            assert len(per_ensemble_log_likelihood[0]) == data.n_ensemble

            per_ensemble_ll.append(np.array(per_ensemble_log_likelihood)) # (n_labels, n_ensemble)
            
            curr_label_ll = [self.combine_ensemble_ll(_, weights=dp["similarity_with_demos"]) for _ in per_ensemble_log_likelihood]
            assert len(curr_label_ll) == len(dp["options"])
            
            prediction_idx = np.argmax(curr_label_ll)
            prediction = dp["options"][prediction_idx]
            predictions.append(prediction.strip())

            prob = np.exp(curr_label_ll) / np.sum(np.exp(curr_label_ll))
            per_label_probs.append(prob)
        
        
        return {"predictions": predictions, "per_label_probs": per_label_probs, "per_ensemble_ll": per_ensemble_ll}
    ...
    def combine_ensemble_ll(self, ll, weights=None):
        if self.ensemble_method == "poe":
            return np.average(ll, weights=weights)
        elif self.ensemble_method == "moe": # mixture of experts
            return np.average(np.exp(ll), weights=weights)
        elif self.ensemble_method == "max":
            if weights is not None: ## weighted max
                weights = np.array(weights)
                weights = weights / np.sum(weights)
                return np.max(np.exp(ll) * weights)
            else:
                return np.max(ll)
        elif self.ensemble_method == "min":
            return np.min(ll)
        elif self.ensemble_method == "most_similar":
            most_sim_idx = np.argmax(weights)
            return ll[most_sim_idx]
        elif self.ensemble_method is None:
            return ll[0]
        else:
            raise ValueError("Unknown ensemble method %s" % self.ensemble_method)
